...
class CrossMap(object):
	...
	def generate_linewords_table(self, tweets, voca):
		file_uw = 'GraphEmbed/linewords-table-uw.txt'
		file_lw = 'GraphEmbed/linewords-table-lw.txt'
		file_tw = 'GraphEmbed/linewords-table-tw.txt'
		...
		for tweet in tweets:
			...
			tw = []
			uw.append(tweet.uid)
			lw.append(self.lClus.predict([tweet.lat, tweet.lng]))
			tw.append(self.tClus.predict([tweet.ts]))
            
			words = [w for w in tweet.words if w in voca]
			for word in words:
				uw.append(word)
				lw.append(word)
				tw.append(word)
			...
			lw.append("\n")
			tw.append("\n")
			f_uw.writelines("\t".join(str(w) for w in uw))
			f_lw.writelines("\t".join(str(w) for w in lw))
			f_tw.writelines("\t".join(str(w) for w in tw))
	...
	def fit(self, tweets, voca):
		...
		print('spatial cluster num: {}'.format(len(self.lClus.centroids)))
		self.tClus.fit(times)
		print('temporal cluster num: {}'.format(len(self.tClus.centroids)))
		self.generate_linewords_table(tweets, voca)
	...
	def prepare_training_data(self, tweets, voca):
		...
		for tweet in tweets:
			...
			if 'c' in nts and c not in self.pd['category_list']:
				...
				tagged -= 1
				continue
			c2n = [str(c), str(l), str(t)]
			for w in words:
				c2n.append(str(w))
			cat2nodes.append(c2n)
			for nt1 in nts:
			...
			for w1, w2 in itertools.combinations(words, r=2):
				if w1!=w2:
					et2net['ww'][w1][w2] += 1
					et2net['ww'][w2][w1] += 1
			# if len(tweet.mention):
			# 	for user in tweet.mention:
			# 		nt2nodes['u'].add(user)
			# 		et2net['uu'][tweet.name][user] += 1
			# 		et2net['uu'][user][tweet.name] += 1
		c2n_file = self.pd['cat2node_file']
		fo = open(c2n_file, 'w')
		for c2n in cat2nodes:
			fo.write(' '.join(c2n)+'\n')
		fo.close()

		print ('Graph Construction Done, skip {} tweets, {} of {} is tagged'.format(empty_text, tagged, len(tweets)))
	...
	def write_graph(self, nt2nodes, et2net):
		...
		print('There are {} nodes in total!'.format(node_num)) 
		...
		print('edge types are {}'.format(et2net.keys()))
		return node_id
