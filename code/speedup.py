...
class HighOrder(object):
    ...
    def fit(self, thread_num):
        self.embed_algo = GraphEmbed(self.pd)
        self.nt2vecs, self.nt2cvecs = self.embed_algo.fit(self.nt2nodes, self.et2net, self.pd['epoch']*self.pd['train_size'], thread_num)
...
class GraphEmbed(object):
    ...
    def fit(self, nt2nodes, et2net, sample_size, thread_num):
    	self.write_line_input(nt2nodes, et2net)
    	self.execute_line(sample_size, thread_num)
    ...
    def execute_line(self, sample_size, thread_num):
        command = ['./hin2vec']
        ...
        command += ['-threads', str(thread_num)]
...
if __name__ == "__main__":
    ...
    train_time = []
    for k in threads:
        print("Start training with {} threads!".format(k))
        start_time = time.time()
        model.fit(k)
        elapsed_time = time.time()-start_time
        train_time.append(elapsed_time)
        print("Model training done, elapsed time {}s".format(elapsed_time))   
        model.mr_predict()