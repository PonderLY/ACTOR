...
public:
	...
	friend class line_words;
    friend class line_trainer_edge;
	friend class line_trainer_words;
...
{
protected:
	char linewords_file[MAX_STRING];
	line_node *node_u, *node_v;
	int edge_type;
...
public:
	...
	void init(char edge_type, line_hin *p_hin, line_words *p_words, int negative);
	void train_sample_word_first(real alpha, real *_error_vec, double(*func_rand_num)(), unsigned long long &rand_index, int second_order, int cnt, bool update_wei);
	void train_sample_word_last(real alpha, real *_error_vec, double(*func_rand_num)(), unsigned long long &rand_index, int second_order, int cnt, bool update_wei);
};
